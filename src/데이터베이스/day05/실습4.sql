...
select genre , sum(stock) 총몇권 from books group by genre;
-- [문제 5] orders 테이블에서 각 도서번호(book_id)별로 총 몇 권이 주문되었는지 조회하세요.
select book_id , sum( order_qty ) 총몇권 from orders group by book_id;
...
select customer , sum(order_qty) from orders 
	group by customer having customer = '김민준' or customer = '이서연' and sum(order_qty) >= 4; 
...
select book_id , count(book_id) from orders group by book_id having count(book_id) >= 3;
...
select author  , sum( stock ) from books group by author having sum( stock ) >= 2; 
...
select genre , sum( stock ) as 재고 from books group by genre having genre = '소설';
...
select customer , max(order_date) , min(order_date) from orders group by customer;
...
select customer , count(book_id) from orders group by customer order by customer desc limit 1 ;
